...
$folderNames = @("TriHardPrivate", "TriHardShared", "TriHardIT", "TriHardConsultant", "TriHardSales", "TriHardManagement", "TriHardAccounting")
# Create the folders
...
foreach ($folderName in $folderNames) {
    ...
    # Remove existing access rules for TriHardIT
    $acl.Access | Where-Object { $_.IdentityReference -match "TriHardIT" } | ForEach-Object { $acl.RemoveAccessRule($_) }
    $acl.Access | Where-Object { $_.IdentityReference -match "Domain Admins" } | ForEach-Object { $acl.RemoveAccessRule($_) }

    # Grant FullControl to TriHardIT explicitly
    $accessRule = New-Object System.Security.AccessControl.FileSystemAccessRule("TriHardIT", "FullControl", "ContainerInherit,ObjectInherit", "None", "Allow")
    ...
    $names = @("TriHardManagement")
    $permissions = "Modify"
    switch ($folderName) {
        "TriHardConsultant" {
            $names += "TriHardConsultant"
        }
        "TriHardSales" {
            $names += "TriHardSales"
        }
        ...
        "TriHardAccounting" {
            $names += "TriHardAccounting"
        }
        "TriHardPrivate" {
            $names += "TriHardConsultant", "TriHardSales", "TriHardAccounting"
        }
        "TriHardShared" {
            $names += "TriHardConsultant", "TriHardSales", "TriHardAccounting"
        }
...
Add-Printer -Name "TriHardPrinter" -DriverName "Microsoft OpenXPS Class Driver 2" -PortName "10.6.68.179"
